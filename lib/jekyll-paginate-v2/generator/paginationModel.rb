  ...
  module PaginateV2::Generator
    ...
    class PaginationModel
      ...
      def paginate(template, config, site_title, all_posts, all_tags, all_categories, all_locales)
        ...
        # Now for each pagination page number create a page and configure the ranges for the collection
        (1..total_pages).each do |cur_page_nr|
          ...
          # 3. Create the pager logic for this page number
          pager = Paginator.new(config['per_page'], first_index_page_url, paginated_page_url, using_posts, cur_page_nr, total_pages, indexPageName, indexPageExt)

          # Create the url for the new page, make sure we prepend any permalinks that are defined in the template page
          pager_path = pager.page_path
          ...
          if cur_page_nr > 1
            ...
            # Add any user-specified title suffix to all the pages except the first
            tmp_title = template.data['title'] || site_title
            if config.has_key?('title')
              tmp_title = "#{Utils.format_page_title(config['title'], tmp_title, cur_page_nr, total_pages)}"
            end
          ...
          # Store the automatically generated page in an internal list for later referencing if we need to generate a
          # pagination number path later on
          newpages << newpage
