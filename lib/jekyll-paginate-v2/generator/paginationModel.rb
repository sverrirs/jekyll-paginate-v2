    ...
    class PaginationModel
        ...
        templates.each do |template|
          ...
          if template.data['pagination'].is_a?(Hash)
            ...
            if( template_config['enabled'] )
              ...
              all_filtered_collection = PaginationIndexer.index_posts_by(all_posts, template_config['filter_coll_by']) if template_config['filter_coll_by']
              ...
              self.paginate(template, template_config, site_title, all_posts, all_tags, all_categories, all_locales, all_filtered_collection)
            end
          end
        end #for
      ...
      def _debug_print_config_info(config, page_path)
        ...
        if @debug
          ...
          puts f + "  Filter Coll By: ".ljust(r) + (config['filter_coll_by'].nil? ? "[Not Set]" : config['filter_coll_by'].to_s)
      ...
      #
      def paginate(template, config, site_title, all_posts, all_tags, all_categories, all_locales, all_filtered_collection)
        ...
        if config['filter_coll_by']
          using_posts = PaginationIndexer.read_config_value_and_filter_posts(config, config['filter_coll_by'], using_posts, all_filtered_collection, should_union)
          self._debug_print_filtering_info(config['filter_coll_by'].capitalize, before, using_posts.size.to_i)
        end
